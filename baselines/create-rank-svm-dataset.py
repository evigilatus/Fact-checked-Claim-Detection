...
def get_score(iclaim_id, iclaim, bm25_title, dictionary_title, bm25_vclaim, dictionary_vclaim, bm25_text, dictionary_text, vclaims_list, index, search_keys, size=10000):
    ...
    not_best_docs = sorted_scores[0:19149]
    random_docs = random.sample(not_best_docs, 100)
    best_docs = sorted_scores[-100:]
    best_score = []
    for best_doc in best_docs:
        best_score.append((
            ...
            scores[best_doc]))
    for random_doc in random_docs:
        best_score.append((
            iclaim_id,
            vclaims_list[random_doc]['vclaim_id'],
            scores_title[random_doc],
            scores_vclaim[random_doc],
            scores_text[random_doc],
            scores[random_doc]))
    return best_score
...
def get_scores(iclaims, vclaims_list, index, search_keys, size):
    ...
    for iclaim_id, iclaim in iclaims:
        score = get_score(iclaim_id, iclaim, bm25_title, dictionary_title, bm25_vclaim, dictionary_vclaim, bm25_text, dictionary_text, vclaims_list, index, search_keys=search_keys, size=size)
        scores.extend(score)
    return scores
...
def run_baselines(args):
    ...
    df = pd.DataFrame(scores, columns=['iclaim_id', 'vclaim_id', 'title', 'vclaim', 'text', 'scores'])
    print(df.sample(100))
    print(df.describe())
