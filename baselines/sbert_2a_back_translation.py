...
def get_score(iclaim_encodding, vclaims_list, vclaims_tr_list, vclaim_encodings, vclaim_encodings_tr, index, search_keys, size=10000):
    score = {}

    for i, (vclaim, vclaim_tr) in enumerate(zip(vclaim_encodings, vclaim_encodings_tr)):
        results = util.semantic_search(iclaim_encodding, vclaim,  top_k = 1)
        results_tr = util.semantic_search(iclaim_encodding, vclaim_tr,  top_k = 1)
        vclaim_id = vclaims_list[i]['vclaim_id']
        vclaim_id_tr = vclaims_tr_list[i]['vclaim_id']

        if vclaim_id != vclaim_id_tr:
            print(vclaim_id, vclaim_id_tr, 'DIFFERENT')

        score_original = calc_score(results)
        score_tr = calc_score(results_tr)
        score[vclaim_id] = max(score_original, score_tr)
...
def get_scores(iclaims, vclaims_list, vclaims_tr_list, index, search_keys, size):
    ...
    logging.info("All vclaims encoded successfully.")
     ...
     # Compute the encodings for all translated vclaims in all texts
    texts_tr = [preprocess_vclaims(vclaim)  for vclaim in vclaims_tr_list]
    vclaim_encodings_tr = [sbert.encode(sent_tokenize(text)) for text in texts_tr]
    ...
    for iclaim_id, iclaim in iclaims:
        score = get_score(iclaims_encodings[counter], vclaims_list, vclaims_tr_list, vclaim_encodings, vclaim_encodings_tr, index, search_keys=search_keys, size=size)
        counter += 1
...
def run_baselines(args):
    ...
    vclaims_tr, vclaims_tr_list = load_vclaims(args.vclaims_translated_dir_path)
    ...
    scores = get_scores(iclaims, vclaims_list, vclaims_tr_list, index, search_keys=args.keys, size=args.size)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--vclaims-translated-dir-path", "-tr",  required=True, type=str,
